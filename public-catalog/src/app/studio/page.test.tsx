...
describe('StudioPage calendar date range', () => {
  ...
  it('sends visibility with gallery saves while omitting spoofable userId and keeps server ownership fields in cache', async () => {
    const calls: Array<{ url: string; body: Record<string, unknown> | null }> = [];
        ...
        return {
          ...
          json: async () => ({
            ...
            item: {
              ...
              prompt: 'marketplace-ready quantum art',
              userName: 'Test User',
              catalogName: "Test's Catalog",
    ...
    await waitFor(() => {
      ...
      expect(galleryCall?.body?.deviceId).toBe('device-1');
      expect(galleryCall?.body).not.toHaveProperty('userId');
    });

    await waitFor(() => {
      const cached = JSON.parse(localStorage.getItem('ft.gallery.cache') || '[]') as Array<Record<string, unknown>>;
      expect(cached[0]?.userId).toBe('server-user-1');
      expect(cached[0]?.visibility).toBe('private');
    });
  });
